...
# === GROUP ===
var _group: Node = null  # EnemyGroup
...
func _on_ready_after_terrain() -> void:
	...
	# Nur zu PATROL wenn wir kein Target haben (könnten bereits ALERT sein)
	if _target == null:
		_state = State.PATROL_IDLE
	else:
		print("[GOBLIN] Ready after terrain with target, staying in state: ", State.keys()[_state])


func _process_ai(delta: float) -> void:
	# Player Detection (außerhalb von bestimmten States)
...
func _process_alert(delta: float) -> void:
	...
	# Target aktualisieren falls noch nicht gesetzt oder ungültig
	if _target == null or not is_instance_valid(_target):
		_target = get_tree().get_first_node_in_group("player")
		print("[GOBLIN] Alert: Target was null, found: ", _target)
	...
	_state_timer -= delta
	if _state_timer <= 0:
		print("[GOBLIN] Alert ended. Target: ", _target, " Valid: ", is_instance_valid(_target) if _target else false)
		
		# Nochmal Target prüfen bevor wir Chase starten
		if _target == null or not is_instance_valid(_target):
			_target = get_tree().get_first_node_in_group("player")
			print("[GOBLIN] Re-acquired target: ", _target)
		
		if _target and is_instance_valid(_target):
			print("[GOBLIN] Entering CHASE")
			_enter_state(State.CHASE)
		else:
			print("[GOBLIN] No target, back to PATROL")
			_enter_state(State.PATROL_IDLE)


func _process_chase(delta: float) -> void:
	if not _target or not is_instance_valid(_target):
		print("[GOBLIN] Chase: Lost target, back to patrol")
		_reset_stuck_detection()
		_enter_state(State.PATROL_IDLE)
		return
	...
	if dist > lose_interest_range:
		print("[GOBLIN] Chase: Target too far (", dist, "), losing interest")
		_reset_stuck_detection()
		_target = null
		_enter_state(State.PATROL_IDLE)
		return
	...
	if dist <= preferred_distance + preferred_distance_tolerance:
		print("[GOBLIN] Chase: Reached preferred distance, entering CIRCLE_IDLE")
		_reset_stuck_detection()
...
func set_target(target: Node3D) -> void:
	...
	# Nur State wechseln wenn wir nicht bereits im Kampf sind
	if _state in [State.PATROL_IDLE, State.PATROL_WALK]:
		_enter_state(State.ALERT)
	elif _state in [State.CIRCLE_IDLE, State.ATTACK_RECOVERY]:
		# Bereits im Kampf - einfach Target aktualisieren
		pass
