@tool
class_name BakedWallRuntime
extends Node3D

## RUNTIME-Node fuer eine gebakte Wand-Verkleidung.
## Pro Modell: ein MultiMeshInstance3D (1 Draw Call/Surface) + CollisionShape3Ds
## (geteilte, vorgekochte Shape) unter EINEM StaticBody3D.
##
...
@export var bake_data: BakedWallData:
	set(value):
		bake_data = value
		if is_inside_tree():
			_rebuild()
...
func _rebuild() -> void:
	...
	_static_body.name = "WallCollision"
	...
	for gi in bake_data.groups.size():
		var group: BakedWallGroup = bake_data.groups[gi]
		if group.multimesh == null:
			continue

		var mmi := MultiMeshInstance3D.new()
		mmi.name = "WallMesh_%d" % gi
